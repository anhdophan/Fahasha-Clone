...
namespace DoAn.Controllers.MoMoPayment.Services
{
    public class MomoService : IMomoService
    {
        ...
        public async Task<MomoCreatePaymentResponseModel> CreatePaymentAsync(OrderInfoModel model)
        {
            ...
            if (string.IsNullOrWhiteSpace(momoApiUrl))
            {
                throw new Exception("MomoApiUrl is not configured properly.");
            }
            // Bạn cũng có thể kiểm tra xem URL có chứa "http://" hoặc "https://" hay không:
            if (!momoApiUrl.StartsWith("http://") && !momoApiUrl.StartsWith("https://"))
            {
                throw new Exception("MomoApiUrl must start with http:// or https://");
            }
